...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NULLIF(NEW.raw_user_meta_data->>'position', ''), 'Психолог'),
  ...
  ELSIF v_admin_created AND COALESCE(NEW.raw_user_meta_data->>'role', '') IN ('admin', 'manager', 'employee') THEN
    v_role := (NEW.raw_user_meta_data->>'role')::public.app_role;
  ELSE
    v_role := 'employee'::public.app_role;
  END IF;
