-- Добавляем колонку is_working в таблицу праздников
-- Если true, то праздник считается рабочим днем (но оплачивается дополнительно)
-- Если false, то праздник выходной

ALTER TABLE public.holidays ADD COLUMN is_working BOOLEAN DEFAULT false;

-- Обновляем права (на всякий случай, хотя таблица уже существует)
GRANT SELECT, INSERT, UPDATE, DELETE ON public.holidays TO authenticated;
GRANT ALL ON public.holidays TO service_role;

-- Заполняем основные праздники РФ на 2026 год
-- Обычно в праздники ОКП работает (2/2 не прерывается), но это "праздничные" дни.
-- По умолчанию ставим false (нерабочие для общего календаря), но админ сможет переключить.
