...
const sections: Section[] = [
  {
    ...
    steps: [
      ...
      "Переключайте режимы просмотра в верхней панели: Месяц, Неделя, День и Год.",
      "В шапке календаря показаны кнопки-счётчики: «Все» и «Группа 1/Группа 2» с количеством сотрудников — нажмите, чтобы оставить в календаре только выбранную группу.",
      "Сотрудники с одинаковым временем смены и обеда объединены в одну карточку: время, кружочки с фото и «+N».",
      "Наведите курсор на карточку — появится список людей со статусами «в системе» или «в чате» и кнопкой «написать».",
      "В режиме «День» и «Неделя» смена отображается в виде вертикального блока на шкале времени 08:00–20:00 с указанием начала, обеда и конца.",
      "Суббота и воскресенье помечены тонкой диагональной штриховкой, если выходные включены администратором.",
      "Цвета и обозначения расшифрованы в легенде над сеткой календаря: смена, обед, отпуск, праздник, завершённая смена, выходной.",
      "Полоса прогресса на карточке смены показывает, какая часть рабочего времени уже отработана.",
      "В тёмной теме завершённые смены выглядят приглушённо, но сохраняют зелёный оттенок.",
    ],
  },
  {
    id: "my-shifts",
  ...
  },
  {
    ...
    steps: [
      ...
      "Новые сообщения появляются сразу у всех участников — обновлять страницу не нужно.",
      "У своих сообщений виден статус: «Отправляется», «Доставлено» или «Не отправлено» с кнопкой «Повторить».",
  ...
  },
  {
    ...
    roles: ["employee", "manager", "admin"],
    steps: [
      "Если отметка включена администратором, в день смены появится кнопка «Был».",
      "Время отметки сохраняется автоматически и видно руководителю.",
      "В автоматическом режиме отметка ставится сама при первом входе в день смены.",
      "Администратор включает функцию и выбирает режим в разделе «Настройки» → «Система».",
    ],
  },
  {
    id: "stats",
    ...
    roles: ["manager", "admin"],
    steps: [
      "Отдельный пункт меню «Статистика» — отпуска и аналитика по сотрудникам.",
  ...
  },
  {
    ...
    roles: ["admin"],
    steps: [
      "Раздел «Управление» → вкладка «Роли и доступы» (видна только администратору).",
  ...
  },
  {
    ...
    roles: ["manager", "admin"],
    steps: [
      "Праздники РФ: полный список, включая перенесённые выходные, памятные даты и День города — любой день можно сделать рабочим или нерабочим.",
      "Выходные: включите субботу и воскресенье как нерабочие и при необходимости назначьте конкретную дату рабочей.",
      "Нормы по месяцам: задайте плановое рабочее время с учётом смен и праздников.",
      "Система: переключатели «Сотрудник может сам создавать смены» и «Отметка присутствия» с выбором режима.",
  ...
  },
  {
    ...
    roles: ["admin"],
    steps: [
      "Отдельный пункт меню «Журнал системы» — доступен только администратору.",
      "Фиксируются входы, ошибки и действия: подтверждение отпусков, активация учётных записей, формирование графика и изменение настроек.",
      "Одинаковые записи сворачиваются в одну строку со счётчиком «N раз»; группировку можно отключить переключателем.",
      "Найденную ошибку или инцидент можно отметить как исправленный — запись получит бейдж «Исправлено» и скроется из активного списка.",
      "Используйте фильтры по типу события, уровню и поиску для разбора инцидентов.",
    ],
  },

  {
    id: "version",
    title: "Версия системы и обновления",
    icon: BookOpen,
    roles: ["employee", "manager", "admin"],
    steps: [
      "Внизу любой страницы указана текущая версия системы.",
      "Нажмите на номер версии, чтобы открыть страницу «Что нового» со списком изменений.",
      "Инструкции в этом разделе обновляются вместе с новыми версиями.",
    ],
  },
];
